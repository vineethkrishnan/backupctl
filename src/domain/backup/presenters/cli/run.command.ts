...
export class RunCommand extends CommandRunner {
  constructor(private readonly runBackup: RunBackupUseCase) {
    super();
  }

  @Option({ flags: '--all', description: 'Run backup for all enabled projects' })
  ...
  private async runSingle(projectName: string): Promise<void> {
    ...
    const [result] = await this.runBackup.execute(new RunBackupCommand({ projectName }));
    this.printResult(result);
    if (result.status === BackupStatus.Failed) { process.exitCode = 1; }
  }

  private async runAll(): Promise<void> {
    console.log('Running backup for all enabled projects...\n');
    const results = await this.runBackup.execute(new RunBackupCommand({ isAll: true }));
    ...
    else if (failed > 0) { process.exitCode = 5; }
  ...
  private printResult(result: BackupResult): void {
    ...
    if (result.errorMessage) {
      ...
      if (result.errorStage) { console.log(`   Stage: ${result.errorStage} | Retries: ${result.retryCount}`); }
    }
  }
  ...
  private handleError(error: unknown): void {
    ...
    if (message.includes('already in progress')) { process.exitCode = 2; console.error(`Error: ${message}`); return; }
