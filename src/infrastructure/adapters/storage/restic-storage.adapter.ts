...
import { RemoteStoragePort, SyncOptions } from '@domain/backup/ports/remote-storage.port';
...
export class ResticStorageAdapter implements RemoteStoragePort {
  ...
  private parseSyncOutput(stdout: string): ResticSummary {
    // Restic --json outputs one JSON message per line; the summary is the last one
