...
describe('notification-formatter', () => {
  ...
  describe('buildDailySummaryEntries', () => {
    it('should build success entries', () => {
      ...
      const result = createResult({ projectName: 'vinelab', dumpResult: dump, syncResult: sync });
  ...
  describe('formatFailureText', () => {
    it('should format failure text with stage and retryable info', () => {
      ...
      const text = formatFailureText('vinelab', error);

      expect(text).toContain('❌ Backup failed — vinelab');
  ...
  describe('formatSuccessText', () => {
    it('should produce plaintext success with all sections', () => {
      const dump = new DumpResult('/data/backups/vinelab.sql.gz', 1024 * 1024, 5000);
      ...
      expect(text).toContain('✅ Backup completed — vinelab');
      expect(text).toContain('DB: vinelab');
  ...
  describe('formatDailySummaryText', () => {
    it('should produce daily summary with mixed results', () => {
      const success = createResult({ projectName: 'vinelab', durationMs: 5000 });
